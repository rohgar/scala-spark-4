...
class StackOverflow extends Serializable {
  ...
  def scoredPostings(grouped: RDD[(Int, Iterable[(Posting, Posting)])]): RDD[(Posting, Int)] = {
    ...
    grouped.map(
      pair => {
        ...
        val question = pair._2.head._1
        val answersArray = pair._2.map(postingsPair => (postingsPair._2)).toArray
        val highestScoreOfAnAnswer = answerHighScore(answersArray)
        (question, highestScoreOfAnAnswer)
      })
  }
  ...
  def vectorPostings(scored: RDD[(Posting, Int)]): RDD[(Int, Int)] = {
    ...
    val intermediate = scored.map(pair => {
      val question = pair._1
      val distanceOption = firstLangInTag(question.tags, langs)
      val distance = firstLangInTag(question.tags, langs).getOrElse(emptyOption)
      val scoreOfHighestRatedAnswer = pair._2
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(Int, Int)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val langLabel: String = {
        val index = vs.groupBy(pair => pair._1).map(pair => (pair._1, pair._2.size)).maxBy(pair => pair._1)._1 / langSpread
        langs(index)
      }
      ...
      val medianScore: Int = {
        val seq = vs.map(x => x._2).toSeq
        val (firstHalfSeq, secondHalfSeq) = seq.sortWith(_ < _).splitAt(seq.size / 2)
        if (seq.size % 2 == 0) (firstHalfSeq.last + secondHalfSeq.head) / 2 else secondHalfSeq.head
      }
